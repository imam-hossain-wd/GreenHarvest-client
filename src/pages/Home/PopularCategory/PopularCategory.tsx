...
const PopularCategory = () => {
  ...
  return (
    <div>
      <h1 className="text-center my-7">Our popular category</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 w-11/12 mx-auto">
        {categories.map((category, index) => (
          <Link
            ...
            className="no-underline text-black border-gray-300 shadow-lg rounded-lg"
            key={index}
          >
            <div className="overflow-hidden m-5">
              <img
                className="transition ease-in-out delay-150  hover:-translate-y-1 hover:scale-110 duration-300 w-full rounded-lg h-56"
                src={category.image}
              />
            </div>
